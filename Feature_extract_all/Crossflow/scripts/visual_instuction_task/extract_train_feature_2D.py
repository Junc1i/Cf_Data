...
def main():
      ...
      try:
        for batch_idx, batch in enumerate(dataloader):
            ...
            batch_data = {
                ...
                'input_image_relative_paths': batch_input_relative_paths,
                'output_image_relative_paths': batch_output_relative_paths,
                'vae_type': '2D',
                'llm': llm_type,
                'resolution': 256,
            }
    ...
    if accelerator.is_main_process:
        ...
        total_remaining = len(dataset) 
        total_source_pairs = total_processed + total_remaining
        progress_pct = (total_processed / total_source_pairs * 100) if total_source_pairs > 0 else 0.0
        ...
        with open(log_file, 'a', encoding='utf-8') as fh:
            ...
            fh.write(f"\n总体进度:\n")
            fh.write(f"  - 总样本对数: {total_source_pairs}\n")
            fh.write(f"  - 已处理总数: {total_processed}\n")
            fh.write(f"  - 待处理数量: {total_remaining}\n")
        ...
        print(f'\n 总体进度统计:')
        print(f'  总样本对数: {total_source_pairs}')
        print(f'  已处理总数: {total_processed}')
        print(f'  待处理数量: {total_remaining}')
