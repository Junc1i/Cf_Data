...
def main():
    ...
    input_image_path = os.getenv('INPUT_IMAGE_PATH', '/storage/v-jinpewang/lab_folder/junchao/crossflow_data/recon_data/trainset/train_03')
    output_image_path = os.getenv('OUTPUT_IMAGE_PATH', '/storage/v-jinpewang/lab_folder/junchao/crossflow_data/recon_data/trainset/train_03_output')
    ...
    with torch.no_grad():
        try:
            for batch_idx, batch in enumerate(dataloader):
                ...
                batch_names = [n for n, v in zip(batch['img_name'], success_list) if v]  # 现在是数字键
